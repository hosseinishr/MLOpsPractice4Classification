...
def encode_features():
    ...
    **NOTE : You can modify the encode_featues function used in heart disease's inference
    ...
    df_features.to_sql(name=DF_FEATURES_TABLENAME, con=cnx, if_exists='replace', index=False)
    df_target.to_sql(name=DF_TARGET_TABLENAME, con=cnx, if_exists='replace', index=False)
...
def get_trained_model():
    ...
    OUTPUT
        ...
        Calculates auc from the test data and log into mlflow run  
    ...
    # mlflow server --backend-store-uri='sqlite:///Assignment/airflow/dags/Lead_scoring_training_pipeline/Lead_scoring_mlflow_production.db' --default-artifact-root="/home/mlruns/" --port=6006 --host=0.0.0.0
    ...
    # read the input data
    cnx = sqlite3.connect(DB_PATH+DB_FILENAME)
    df_features = pd.read_sql('select * from '+DF_FEATURES_TABLENAME, cnx)
    df_target = pd.read_sql('select * from '+DF_TARGET_TABLENAME, cnx)
